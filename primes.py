...
totalPixels = (CANVAS_HEIGHT/(NODE_SIZE*2))**2 if CANVAS_HEIGHT>CANVAS_WIDTH else (CANVAS_WIDTH/(NODE_SIZE*2))**2
master = Tk()
...
def createBox(x, y, NODE_SIZE, fill=colors[0]):
    w.create_rectangle(x-NODE_SIZE, y-NODE_SIZE, x+NODE_SIZE, y+NODE_SIZE, fill=fill)
def isPrime(num):
	if(num<2): 
		return False
	if(num==2): 
...
while((coord["x"]>0 or coord["y"]>0) and (coord["x"]<CANVAS_WIDTH or coord["y"]<CANVAS_HEIGHT)):
	...
	if(not(coord["x"]<0 or coord["y"]<0 or coord["x"]>CANVAS_WIDTH or coord["y"]>CANVAS_HEIGHT)):
		boxCoordinates.append({"x":coord["x"],"y":coord["y"],"fill":colors[1] if isPrime(i) else colors[0]})
	iterator+=1
	if(direction=="right"):
		coord["x"]+=stepSize
	elif(direction=="up"):
